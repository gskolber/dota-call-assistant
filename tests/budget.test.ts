...
test('spokenInWindow counts only the last 60 seconds', () => {
  ...
  engine.tick(128, flags(), state()); // PULL

  assert.equal(engine.spokenInWindow(128), 2);
...
test('the per-minute budget drops low-priority calls once it is exceeded', () => {
  ...
  assert.deepEqual(ids(engine.tick(128, f, state())), ['pull']); // p2, budget free
  ...
  assert.deepEqual(ids(engine.tick(188, f, state())), ['pull']);
});
...
test('a call dropped by the budget does not consume budget itself', () => {
  ...
  engine.tick(128, f, state()); // PULL spoken
  assert.equal(engine.spokenInWindow(128), 1);
