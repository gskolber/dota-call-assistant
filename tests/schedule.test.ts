...
test('STACK fires 5s before :53 of every minute, i.e. at 108, 168, 228…', () => {
  assert.deepEqual(firesBetween('stack', 0, 300), [108, 168, 228, 288]);
});

test('STACK respects its 1:00–30:00 window', () => {
  // the first stack is 1:53, not 0:53 — the window opens at 60
  assert.equal(firesBetween('stack', 0, 120)[0], 108);
  // the last one is 29:53 (clock 1793), spoken at 1788; 30:53 is outside
  assert.deepEqual(firesBetween('stack', 1700, 1900), [1728, 1788]);
});
...
test('role filtering, event by event, at the roles each one declares', () => {
  ...
  assert.deepEqual(firesBetween('stack', 0, 120, 'POS 1'), [108]);
